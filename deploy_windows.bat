...
if exist "%APP_DIR%\requirements.txt" (
    ...
    echo [WARN] Keine requirements.txt gefunden. Installiere Mindestpakete...
    pip install streamlit pandas openpyxl numpy --quiet
    echo [OK]   Mindestpakete installiert.
)
echo.
...
:: ============================================================
::  SCHRITT 7b: Demo-Daten generieren (falls keine SAP-Dateien)
:: ============================================================
if not exist "%DATA_DIR%\input\SAP_OPOS_Vertrieb.xlsx" (
    echo [INFO] Keine SAP-Exportdateien in %DATA_DIR%\input\ gefunden.
    echo [INFO] Generiere Demo-Daten fuer erste Inbetriebnahme...
    if exist "%APP_DIR%\data\dummy\generate_dummy_data.py" (
        "%VENV_DIR%\Scripts\python.exe" "%APP_DIR%\data\dummy\generate_dummy_data.py"
        echo [OK]   Demo-Daten generiert. Dashboard zeigt Beispieldaten.
        echo [INFO] Echte SAP-Dateien spaeter nach %DATA_DIR%\input\ kopieren.
    ) else (
        echo [WARN] generate_dummy_data.py nicht gefunden - uebersprungen.
    )
) else (
    echo [OK]   SAP-Exportdateien gefunden in %DATA_DIR%\input\
)
echo.
